...
func Simulation_candidats_QV(out io.Writer, outQV io.Writer) {
	...
	*/
	...
	resultsQV := VoteInstance.GetResultsQuadraticVoting()

	s = "%"
	fmt.Fprintf(outQV, "digraph network_activity {\n")
	fmt.Fprintf(outQV, "labelloc=\"t\";")
	fmt.Fprintf(outQV, "label = <Votation QV Diagram of %d nodes.    Results are Macron = %.4v %s, Trump = %.4v %s, JeanMi = %.4v %s et Obama = %.4v %s,<font point-size='10'><br/>(generated: %s)<br/> Il y a %v YesVoter, %v Threshold Voters, %v Non responsibleVoter, %v ResponsibleVoter, %v IndecisiveVoter and %v NormalVoter</font>>; ",
		len(voters)+2, resultsQV["Macron"], s, resultsQV["Trump"], s, resultsQV["JeanMi"], s, resultsQV["Obama"], s, time.Now(), counterYesVoter, counterThresholdVoter, counterNonResponsibleVoter, counterResponsibleVoter, counterIndecisiveVoter, counterNormalVoter)
	fmt.Fprintf(outQV, "graph [fontname = \"helvetica\"];")
	fmt.Fprintf(outQV, "node [fontname = \"helvetica\" area = 10 fillcolor=gold];")
	fmt.Fprintf(outQV, "edge [fontname = \"helvetica\"];\n")
