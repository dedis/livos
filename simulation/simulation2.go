...
func Simulation2(out io.Writer) {
	...
	for i := 0; i < randomNumOfUser; i++ {
		...
		case chooseType < 70:
			...
			voters = append(voters, &user)
		case chooseType < 90:
			var user, err = VoteSystem.NewUser("user"+strconv.FormatInt(int64(i), 10), make(map[string]voting.Liquid), make(map[string]voting.Liquid), histoChoice, voting.NonResponsibleVoter)
			if err != nil {
				xerrors.Errorf(err.Error())
			}
			voters = append(voters, &user)
